...
模型还可以使用 `Bash` 和 `Python` 在 workspace 内执行命令。`Bash` 默认 cwd 是 workspace，显式 `cwd` 也必须位于 workspace 内；`pwd/ls/find/rg/grep/cat/head/tail/wc/stat/file/du/tree/date` 等只读低风险命令会自动执行。写入、删除、安装、网络、git mutation、权限变更、后台进程、环境变量导出、shell 组合和重定向等高风险 Bash 需要交互确认。未知命令默认需要确认。`Python` 会写入 `workspace/.neo-agent/tmp/python-*.py` 后执行，默认每次都需要确认。
...
- 优先调用 OpenViking `/mcp` 的 `health/search/list/remember/forget`。
