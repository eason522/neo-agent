...
联网工具默认阻止 localhost、内网 IP、链路本地地址、私有地址和配置中拒绝的域名。`WebFetch`、`web extract/map/crawl` 请求 Tavily 前会读取目标站点 `robots.txt`，命中 `Disallow` 时拒绝继续；可用 `NEO_AGENT_WEB_RESPECT_ROBOTS_TXT=0` 关闭 robots 检查，但不建议长期关闭。
