...
- 过渡版小模型联网规划器，用于自动判断是否需要搜索、验证、读取网页和沿用上一轮问题
- 按上下文预算保留 REPL 会话历史
- GitHub `main` 分支同步
...
- [ ] 按 CC-Source 重构联网能力：WebSearch/WebFetch 作为标准 tool 暴露给主模型，由主循环处理 tool_use/tool_result，而不是预先把搜索结果塞进上下文
- [ ] 为联网工具补齐 CC-Source 风格的工具 schema、工具 prompt、只读标记、权限/域名策略、结果预算、来源要求和日志脱敏
- [ ] 为 MCP 工具执行添加安全调用协议
- [ ] 针对高风险工具添加权限确认
