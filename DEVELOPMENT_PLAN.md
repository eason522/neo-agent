...
- skill 发现和自动创建的基础框架
- skill 生命周期命令：`neo skill list/show/create/edit/delete`，REPL 支持 `/skill list/show/path/edit/delete/create`
- skill 安装、导入、打包、显式调用、项目级/全局级区分、热加载和使用统计仍未完成；这是 M3 当前最高优先级缺口
...
- [ ] 添加 `neo skill install <path|url>`，支持从单个 `.md` 安装为标准 `SKILL.md`
- [ ] 添加 `neo skill install <zip>`，支持从 zip skill 包安装，并做 zip-slip、防覆盖、大小、文件数量和 manifest 校验
- [ ] 添加 `neo skill export <name>`，把 skill 打包为可分享 zip
- [ ] 添加 `neo skill validate <path|name>`，校验 `SKILL.md` 必填字段、frontmatter、触发词、文件大小和潜在危险内容
- [ ] 支持 `--scope user|project`，区分全局用户 skill 和项目本地 skill
- [ ] 支持 `--overwrite`、`--dry-run` 和安装预览，避免误覆盖已有 skill
- [ ] 兼容 CC-Source 风格：目录格式 `skill-name/SKILL.md`、单 `.md` 导入、插件 manifest 中的 `skillsPath/skillsPaths`
...
| skill | `skills/loadSkillsDir.ts`、`tools/SkillTool`、`tools/SkillTool/prompt.ts`、`utils/suggestions/skillUsageTracking.ts`、`utils/skills/skillChangeDetector.ts`、plugin `skillsPath/skillsPaths`、zip cache/install helpers | 不充分。已有 SKILL.md 发现、自动创建、CLI 生命周期命令和 REPL 管理命令；但缺安装 `.md`/`.zip`、导出、校验、显式 Skill tool、上下文预算化 listing、usage tracking、热加载、项目/全局 scope、plugin skill 导入和安装安全边界。 | M3 优先补 install/validate/export/scope，再补 Skill tool、usage tracking 和热加载。 |
...
审查结论：当前最明显的不合规点是“主 agent loop 和工具循环曾内嵌在 `NeoAgent` 中”。已立即校正为最小 `QueryEngine` / `ToolRunner` 分层，并已把 MCP 工具接入该 loop，同时补上默认只读权限保护、deferred ToolSearch、resource 工具和 REPL 一次性权限确认。仍不充分的模块主要是 sub-agent、REPL、自动 compact 和 skill 生命周期；MCP 仍需继续补 always allow/deny 持久化规则、远程 MCP 和更完整权限 UI。开发这些模块时不得继续做孤立实现，必须先对照 CC-Source 对应源代码。
