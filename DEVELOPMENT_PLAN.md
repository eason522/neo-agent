...
| skill | `tools/SkillTool`、`commands/skills`、plugin/skill discovery、`skills/loadSkillsDir.ts` | 部分符合。已有 SKILL.md 发现、自动创建、CLI 生命周期命令和 REPL 管理命令；但还缺使用统计、显式 invoke 工具、动态发现、项目/全局 skill 区分和 skill 改进建议。 | M3 继续按 CC-Source skill 生命周期补齐。 |
