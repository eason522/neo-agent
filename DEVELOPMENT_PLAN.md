...
- [x] 添加中断/取消行为
- [ ] 添加更丰富的消息渲染
- [x] 添加输入历史和多行编辑，支持 `/multi`、行尾 `\`，并尽力兼容 `Ctrl+Enter` / `Alt+Enter` / `Ctrl+J` 快捷换行
