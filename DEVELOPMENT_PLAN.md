...
- skill 显式调用、上下文预算化 listing、插件 manifest 导入、热加载和使用统计仍未完成；这是 M3 后续最高优先级缺口
...
- [ ] 兼容 CC-Source plugin manifest 中的 `skillsPath/skillsPaths`
- [ ] 把 Skill 作为标准 tool 接入 `QueryEngine`，让模型显式调用 skill，而不是只把匹配到的 skill 摘要塞进 system prompt
- [ ] 为 skill listing 增加上下文预算，只暴露名称、描述和 when-to-use，调用时再加载完整正文
- [ ] 记录 skill 使用次数、最近使用时间、成功/失败信号，并用于排序和后续建议
- [ ] 添加 skill 文件变更检测或轻量 reload，避免修改后必须重启 neo
...
neo 已完成文件型 skill 生命周期第一阶段：全局和项目 scope、`SKILL.md` 目录格式、单 `.md`、标准目录、`.zip` 安装、导出、校验、覆盖保护、dry-run、大小/文件数限制和 zip-slip 防护。仍缺失的核心可用能力是：plugin manifest 的 `skillsPath/skillsPaths` 导入、显式 Skill tool、上下文预算化 listing、usage tracking、热加载和更完整的安装安全边界。这些继续留在 M3，下一步优先做 Skill tool，而不是继续扩大自动创建逻辑。
...
| skill | `skills/loadSkillsDir.ts`、`tools/SkillTool`、`tools/SkillTool/prompt.ts`、`utils/suggestions/skillUsageTracking.ts`、`utils/skills/skillChangeDetector.ts`、plugin `skillsPath/skillsPaths`、zip cache/install helpers | 部分符合。已有 SKILL.md 发现、自动创建、CLI 生命周期命令和 REPL 管理命令；已补 `.md`/目录/`.zip` 安装、导出、校验、项目/全局 scope、dry-run、覆盖保护和 zip-slip 防护；但缺显式 Skill tool、上下文预算化 listing、usage tracking、热加载、plugin skill 导入和更完整安装安全边界。 | M3 下一步补 Skill tool，再补 usage tracking、热加载和 plugin manifest 导入。 |
...
审查结论：当前最明显的不合规点是“主 agent loop 和工具循环曾内嵌在 `NeoAgent` 中”。已立即校正为最小 `QueryEngine` / `ToolRunner` 分层，并已把 MCP 工具接入该 loop，同时补上默认只读权限保护、deferred ToolSearch、resource 工具和 REPL 一次性权限确认。仍不充分的模块主要是 sub-agent、REPL 和 Skill tool；MCP 仍需继续补 always allow/deny 持久化规则、远程 MCP 和更完整权限 UI。开发这些模块时不得继续做孤立实现，必须先对照 CC-Source 对应源代码。
...
参考 CC-Source `skills/loadSkillsDir.ts`、`SkillTool` prompt 和 skill change/usage tracking，neo 第一批先补 `~/.neo-agent/skills/<name>/SKILL.md` 的生命周期管理：CLI 提供 `neo skill list/show/create/edit/delete`，REPL 提供 `/skill list/show/path/edit/delete/create`。`edit` 只在交互式 TTY 且设置了 `VISUAL` 或 `EDITOR` 时打开编辑器，非交互模式只输出文件路径，避免脚本卡死。当前仍未做 CC-Source 的 skill invoke tool、使用统计、动态热加载、项目本地 skill 和 skill 改进建议，继续放在 M3 后续任务中。
...
本阶段已经补 zip-slip、路径穿越、文件数量、单文件大小、包大小、symlink 跳过和危险 shell 片段警告；仍未实现 CC-Source plugin manifest `skillsPath/skillsPaths` 导入、Skill tool 显式调用、usage tracking、hot reload 和 hook 执行能力。后续不得把 skill 继续当作静态 system prompt 拼接，应优先接入 `QueryEngine` 的标准 tool loop。
