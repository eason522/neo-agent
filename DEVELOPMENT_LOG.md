...
- [PHASE2_DEVELOPMENT_PLAN.md](./PHASE2_DEVELOPMENT_PLAN.md)
- [PHASE2_DEVELOPMENT_LOG.md](./PHASE2_DEVELOPMENT_LOG.md)

二阶段第一版已完成 workspace 完整文件管理、Bash/Python 执行工具、OpenViking 主记忆存储、Ink TUI 入口层和长工具调用截断恢复。完整实施记录以后写入二阶段专项日志，避免继续把总日志撑得过长。

本文档用于持续跟踪 neo-agent 的开发状态、优先级、待办事项和关键决策。每次做有意义的功能变更前后，都要同步更新这份文档，方便后续恢复上下文和回顾进度。

## 2026-05-26：补充反馈修正收敛原则

用户指出：对于用户提出的反馈和开发中发现的问题，修正时不能无限发散，导致修正支线无法收敛。这个问题在前几轮 P1 收口中已经出现过：每完成一个问题，又把相邻改进继续加回当前优先级，导致计划边界变模糊。

已修正 [DEVELOPMENT_PLAN.md](./DEVELOPMENT_PLAN.md)：

- 在最高原则中新增“反馈修正必须控制范围”。
- 新增“反馈修正收敛规则”。
- 明确只处理当前反馈直接目标和阻塞验证的问题。
- 非阻塞体验改进、架构想法、可选增强和新能力，只记录到计划、日志或后续待办，不回填当前修正支线。

后续处理用户反馈时，必须先收束目标，再修正，再验证，再结束当前支线。
...
### 2026-05-26：WebFetch/Tavily URL 与下载预检第一阶段

继续推进 P1“Web 工具补更完整站点限制策略、重定向/下载预检和更细粒度进度”。先对照 CC-Source：

- `tools/WebFetchTool/utils.ts`：WebFetch 会限制 URL 长度，拒绝 URL 用户名/密码，HTTP 升 HTTPS，做域名预检，手动处理重定向，限制最大下载体积，并缓存结果。
- `tools/WebFetchTool/WebFetchTool.ts`：跨域重定向不会自动跟随，而是返回明确的 redirect message，让模型用新 URL 再次请求。
- `tools/WebFetchTool/prompt.ts`：prompt 明确 private/authenticated URL 会失败，MCP 专用工具优先，跨域重定向要重新发起请求。
- `utils/settings/toolValidationConfig.ts`：WebFetch 权限是 domain 级，而不是完整 URL 级。

neo 使用 Tavily 作为 WebSearch/WebFetch 后端，不直接下载网页正文，因此没有复制 CC-Source 的 HTML-to-markdown、Haiku 二次摘要和二进制落盘流程。本轮做轻量 preflight：

- URL 策略补齐 2000 字符长度限制、用户名/密码拒绝、单标签非公开主机拒绝；保留 HTTP 自动升 HTTPS、私网/本地域名阻断和 allowed/blocked domain 规则。
- 新增 `src/web/webPreflight.ts`：在 Tavily extract/map/crawl 前发起 `HEAD` 预检，`redirect: manual`，最多 10 次同域/www 重定向。
- 跨域、降级协议、端口变化或带凭据的重定向会被拒绝，并提示用户如确认需要则用重定向 URL 重新 WebFetch。
- `content-length` 超过 10MB 会被预检拒绝；PDF/图片/音视频/zip 等二进制 content-type 会写入 warning；401/403 和异常 HTTP 状态也会写入 warning。
- Tavily extract/crawl/map 输出会合并 preflight warnings；Web 工具 prompt 补充重定向和 authenticated/private URL 使用说明。
...
### 2026-05-26：手动 `/compact` 和 compact/resume 状态可见

继续推进 P1“增加手动 `/compact`，并让 compact/resume 的状态在 REPL 中可见”。先对照 CC-Source：

- `commands/compact/compact.ts`：`/compact [instructions]` 可手动压缩会话，并允许用户追加自定义压缩要求。
- `services/compact/compact.ts`：压缩期间会发出 `compacting` 状态，使用无工具的小模型/forked agent 生成摘要。
- `hooks/useRemoteSession.ts` 和 session storage：compact 完成写入 boundary，resume 时只恢复 boundary 后消息并保留摘要。

neo 已有自动 compact、transcript compact boundary 和 resume snapshot。本轮补齐用户可控入口和 REPL 可见状态：

- `ConversationHistory.compact()` 支持强制手动压缩，绕过自动 compact 开关和阈值；压缩 prompt 支持用户自定义要求，fallback 摘要也会保留这些要求。
- `NeoAgent.compactConversation()` 统一发出 compact 状态事件，成功时写入 `compact` transcript entry，metadata 标记 `manual`、摘要长度、保留消息数和自定义要求长度。
- REPL 新增 `/compact [说明]`；`/status` 展示当前 history 消息数、字符预算和 compact 摘要长度；`/resume` 在恢复到 compact boundary 时显示明确提示。
- smoke 覆盖手动 compact、自定义要求、NeoAgent transcript boundary/status events，以及 REPL 空会话 `/compact` 的稳定返回。
