...
export function getWebToolPrompt(): string {
  return [
    ...
    '- WebFetch 会先做 URL/重定向/下载预检。跨域或降级重定向不会自动跟随；如果工具给出重定向 URL，只有在确认需要时才用该 URL 重新读取。',
    '- WebFetch 不适合读取需要登录的 Google Docs、Confluence、Jira、GitHub 私有页面等；优先寻找专用 MCP 或本地 CLI 工具。',
